...
dict_magic = {150: "Doll", 250: "Wooden train", 300: "Teddy bear", 400: "Bicycle"}
craft_dict = {}

while len(materials_for_crafting_toys) > 0 and len(magic_level) > 0 and len(dict_magic) > 0:
    ...
    if current_sum not in dict_magic and current_sum > 0:
        material += 15
        materials_for_crafting_toys.append(material)
        continue
    elif current_sum in dict_magic and current_sum > 0:
        if dict_magic[current_sum] not in craft_dict:
            craft_dict[dict_magic[current_sum]] = 0
        craft_dict[dict_magic[current_sum]] += 1
        continue
