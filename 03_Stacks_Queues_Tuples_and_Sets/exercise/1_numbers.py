...
for row in range(command_lines):
    ...
    current_numbers = set([int(num) for num in command[1:]])

    if command[0] == "Add First":
        [first_sequence.add(num) for num in current_numbers]
    elif command[0] == "Add Second":
        [second_sequence.add(num) for num in current_numbers]
    elif command[0] == "Remove First":
        [first_sequence.remove(num) for num in current_numbers if num in first_sequence]
    elif command[0] == "Remove Second":
        [second_sequence.remove(num) for num in current_numbers if num in second_sequence]
